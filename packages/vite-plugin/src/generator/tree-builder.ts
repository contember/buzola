...
function insertFile(root: FileRouteNode, file: ScannedFile): void {
  ...
  for (const dir of dirs) {
    ...
    let child = current.children.find(c =>
      c.segment === dirInfo.segment && c.isPathlessGroup === dirInfo.isPathlessGroup && c.isLayout
    );

    if (!child) {
      // Find or create a group/directory node
      child = current.children.find(c =>
        c.segment === dirInfo.segment && c.isPathlessGroup === dirInfo.isPathlessGroup && !c.filePath
      );
    }
